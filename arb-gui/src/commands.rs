...
impl CommandDef {
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let arb_perform_key_assignment_sel = sel!(arbPerformKeyAssignment:);
        ...
        let main_menu = match Menu::get_main_menu() {
            Some(existing) => {
                mark_candidates(
                    ...
                    arb_perform_key_assignment_sel,
                );
        ...
        let mut order: Vec<&'static str> = vec!["Arb", "Shell", "Edit", "View", "Window"];
            ...
            for cmd in &commands {
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&cmd.menubar[0], |menu| {
                    if cmd.menubar[0] == "Window" {
                        ...
                        about_item.set_tool_tip("Run `arb` in the active terminal pane");
                        about_item.set_represented_item(RepresentedItem::KeyAssignment(
                            KeyAssignment::EmitEvent("run-arb-cli".to_string()),
                        ));
                        ...
                        settings_item.set_tool_tip("Open and edit user arb.lua");
                        ...
                        reload_item.set_tool_tip("Reload configuration from arb.lua");
                        ...
                        check_update.set_represented_item(RepresentedItem::KeyAssignment(
                            KeyAssignment::EmitEvent("run-arb-update".to_string()),
                        ));
