                ...
                Ok(client) => {
                    ...
                    let res = block_on(executor.run(async move {
                        ...
                        if vers.config_file_path
                            != std::env::var_os("ARB_CONFIG_FILE").map(Into::into)
                        {
                            *self = Publish::NoConnectNoPublish;
                            anyhow::bail!(
                    ...
                    match res {
                        Ok(res) => {
                            log::debug!(
                             ...
                             Use arb start --always-new-process if you do not want this behavior. \
                             Result={:?}",
                                res
                            );
                            Ok(true)
...
fn run_terminal_gui(opts: StartCommand, default_domain_name: Option<String>) -> anyhow::Result<()> {
    ...
    // First, let's see if we can ask an already running Arb instance to do this.
...
fn run() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    {
        unsafe {
            ::windows::Win32::UI::Shell::SetCurrentProcessExplicitAppUserModelID(
                ::windows::core::PCWSTR(wide_string("sh.arb.Arb").as_ptr()),
            )
            .unwrap();
        }
    }
    ...
    let sub = match opts.cmd.as_ref().cloned() {
        ...
        None => {
            ...
            let mut argv = vec!["arb-gui".to_string()];
