...
pub const ICON_DATA: &[u8] = include_bytes!("../../../assets/logo.png");
...
impl TermWindow {
    ...
    fn update_title_impl(&mut self) {
        ...
        let tab_bar_width = if self.show_tab_bar
            ...
            self.config.vertical_tab_bar_width
        } else {
            0
        };
        ...
        let border_cols = border.left.get() / self.render_metrics.cell_size.width as usize;
        ...
        let hovering_in_tab_bar = match &self.current_mouse_event {
            Some(event) => match self.config.effective_tab_bar_position() {
                config::TabBarPosition::Left | config::TabBarPosition::Right => {
    ...
    pub fn perform_key_assignment(
        ...
        match assignment {
            ...
            EmitEvent(name) => {
                if name == "update-arb" || name == "run-arb-update" {
                    pane.writer().write_all(b"arb update\n")?;
                } else if name == "run-arb-cli" {
                    pane.writer().write_all(b"arb\n")?;
                } else if name == "open-arb-config" {
                    crate::frontend::open_arb_config();
                } else {
