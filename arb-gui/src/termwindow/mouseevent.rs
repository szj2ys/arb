...
impl super::TermWindow {
    ...
    pub fn mouse_event_impl(&mut self, event: MouseEvent, context: &dyn WindowOps) {
        ...
        let terminal_origin_x =
            (padding_left + border.left.get() as f32) as isize + tab_bar_pixel_width;
        ...
        let x = (event.coords.x.sub(terminal_origin_x).max(0) as f32)
        ...
        let mut x_pixel_offset = event.coords.x.sub(terminal_origin_x);
        if x > 0 {
