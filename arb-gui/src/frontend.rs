...
pub fn open_arb_config() {
    std::thread::spawn(move || {
        let result = (|| -> anyhow::Result<()> {
            ...
            let arb_bin = exe_dir.join("arb");
            if !arb_bin.exists() {
                anyhow::bail!("could not find arb binary at {}", arb_bin.display());
            }

            let ensure_status = Command::new(&arb_bin)
                .arg("config")
                .arg("--ensure-only")
                .stdin(Stdio::null())
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .status()
                .with_context(|| format!("failed to launch {}", arb_bin.display()))?;
            if !ensure_status.success() {
                anyhow::bail!(
                    "arb config --ensure-only exited with status {}",
                    ensure_status.code().unwrap_or(-1)
                );
            }

            let home = std::env::var("HOME").context("resolve HOME for config path")?;
            let config_path = format!("{home}/.config/arb/arb.lua");
            ...
            let open_status = Command::new("/bin/sh")
                ...
                .stderr(Stdio::null())
                .status()
                .context("open arb.lua in editor")?;
            ...
            ApplicationEvent::PerformKeyAssignment(action) => {
                ...
                match action {
                    KeyAssignment::EmitEvent(event)
                        if event == "update-arb" || event == "run-arb-update" =>
                    {
                        spawn_command(
                            &SpawnCommand {
                                args: Some(vec!["arb".to_string(), "update".to_string()]),
                                ..Default::default()
                            },
                            SpawnWhere::NewWindow,
                        );
                    }
                    KeyAssignment::EmitEvent(event) if event == "run-arb-cli" => {
                        spawn_command(
                            &SpawnCommand {
                                args: Some(vec!["arb".to_string()]),
                                ..Default::default()
                            },
                            SpawnWhere::NewWindow,
                        );
                    }
                    KeyAssignment::EmitEvent(event) if event == "open-arb-config" => {
                        open_arb_config();
                    }
                    KeyAssignment::ReloadConfiguration => {
                        config::reload();
                        refresh_fast_config_snapshot();
                        persistent_toast_notification("Arb", "Configuration reloaded");
                    }
